...
app = Flask(__name__)
app.secret_key = "joanne"  # Replace with your own secret key, needed for session management
...
def generate_data(N, mu, beta0, beta1, sigma2, S):
    ...
    error = np.random.normal(0, np.sqrt(sigma2), N)  # Generate random error term
    ...
    plt.plot(X, model.predict(X.reshape(-1, 1)), color="red", label="Fitted line")  # Plot fitted line
    plt.xlabel("X")
    plt.ylabel("Y")
    plt.legend()
    ...
    for _ in range(S):
        ...
        error_sim = np.random.normal(0, np.sqrt(sigma2), N)  # Simulate error term
...
def hypothesis_test():
    ...
    if p_value is not None and p_value <= 0.0001:
        fun_message = "Wow! Your p-value is great!"
    else:
        fun_message = "Wow! Your p-value sucks."
